...
const messagesPath = isDist
    ...
    : path.join(__dirname, '..', 'assets', 'messages.json');
let messages = { delegation: [], audit: [], conclusion: [] };
try {
            ...
            return {
                tools: [
                        ...
                        inputSchema: {
                            ...
                            properties: {
                                ...
                                dependsOn: {
                                    type: 'array',
                                    items: { type: 'string' },
                                    description: 'List of taskIds this task depends on. Delegation is blocked until these are complete.',
                                },
                                pushFirst: {
                                    type: 'boolean',
                    ...
                    },
                    {
                        ...
                        description: 'Automated Merge Conflict Resolution Engine. Attempts a local git merge and if conflicts exist, isolates them into a new active task for Jules.',
                        inputSchema: {
                            type: 'object',
                            properties: {
                                sourceBranch: {
                    ...
                    },
                    {
                        ...
                        description: 'Discovers JCLAW sessions from GitHub PR metadata and populates the local environment.',
                        inputSchema: {
                            type: 'object',
                            properties: {
                                repository: {
...
Remember: Always start with \`jules_session_info\` and \`jules_screenshot\` to understand the current state before attempting fixes.`,
    ...
    getJclawPun(category) {
        const puns = this.jclawMessages[category] || [
            'The Crimson Orchestrator silently executes its directive.',
        ];
    ...
    async verifyRemoteInstructionFile(repository, branch, filePath) {
        try {
    ...
    async initiateDelegation(args) {
        let { repository, branch, taskId, marker = '@jules', dependsOn = [], pushFirst = true, instruction, instructionFile, } = args;
        ...
        if (dependsOn && dependsOn.length > 0) {
            ...
            for (const dep of dependsOn) {
                ...
                const isArchived = await fs.access(path.join(archiveDir, `${dep}.md`)).then(() => true).catch(() => false);
                if (!isArchived) {
                    unmet.push(dep);
                }
            }
        ...
        const ignoreInstruction = allIgnore.length > 0
    ...
    async getLatestAgentMessage(sessionId) {
        if (!this.config.julesApiKey)
            return undefined;
        try {
            const response = await axios.get(`https://jules.googleapis.com/v1alpha/sessions/${sessionId}/activities?pageSize=10`, {
                headers: { 'x-goog-api-key': this.config.julesApiKey },
            });
            const activities = response.data.activities || [];
            // Find latest agent message
            const agentMsg = activities.find((a) => a.agentMessaged);
    ...
    async checkFeedback(args) {
        ...
        for (const task of activeTasks) {
            if (repository && task.repository.toLowerCase() !== repository.toLowerCase())
                continue;
            try {
                const response = await axios.get(`https://jules.googleapis.com/v1alpha/sessions/${task.id}`, {
                    headers: { 'x-goog-api-key': this.config.julesApiKey },
                });
        ...
        const report = feedbackNeeded
            .map((f) => `### Task ${f.taskId} (${f.repository})\n` +
            `**Branch**: ${f.branch}\n` +
            `**Jules Question**:\n> ${f.question}\n`)
            .join('\n---\n');
        return {
    ...
    async approvePlanViaApi(args) {
        ...
        try {
            ...
            await axios.post(`https://jules.googleapis.com/v1alpha/sessions/${actualTaskId}:approvePlan`, {}, {
                headers: {
                    'x-goog-api-key': this.config.julesApiKey,
                    'Content-Type': 'application/json',
                },
    ...
    async createTaskViaBrowser(args) {
        const { description, repository, branch = 'main', type = 'standard', marker } = args;
    ...
    async getTaskViaApi(args) {
        ...
        try {
            ...
            const response = await axios.get(`https://jules.googleapis.com/v1alpha/sessions/${actualTaskId}`, {
                headers: {
                    'x-goog-api-key': this.config.julesApiKey,
                },
            });
    ...
    async sendMessageViaBrowser(args) {
        const { taskId, message } = args;
        const actualTaskId = this.extractTaskId(taskId);
        const page = await this.getPage();
        try {
    ...
    async approvePlanViaBrowser(args) {
        const { taskId } = args;
        const actualTaskId = this.extractTaskId(taskId);
        const page = await this.getPage();
        try {
    ...
    async resumeTaskViaBrowser(args) {
        const { taskId } = args;
        const actualTaskId = this.extractTaskId(taskId);
        const page = await this.getPage();
        try {
    ...
    async listTasks(args) {
        ...
        const taskList = tasks
            .map((task) => `${task.id}${task.type === 'delegated' ? ' [DELEGATED]' : ''} - ${task.title}\n` +
            `  Repository: ${task.repository}\n` +
            `  Branch: ${task.branch}\n` +
            `  Status: ${task.status}\n` +
    ...
    async analyzeCodeViaApi(args) {
        ...
        try {
            ...
            const response = await axios.get(`https://jules.googleapis.com/v1alpha/sessions/${actualTaskId}/activities?pageSize=20`, {
                headers: {
                    'x-goog-api-key': this.config.julesApiKey,
                },
            });
            ...
            const summary = activities
                .map((a) => {
                const type = Object.keys(a).find((k) => k !== 'createTime' && k !== 'name' && k !== 'description') ||
                    'activity';
                return `- ${type}: ${a.description || a[type]?.description || a[type]?.prompt || ''}`;
            })
                .join('\n');
            // Find ChangeSet artifacts to salvage code
            const changeSets = activities.filter((a) => a.changeSet).map((a) => a.changeSet);
            let patchContent = '';
    ...
    async analyzeCodeViaBrowser(args) {
    ...
    async generateAuditReport(args) {
        const { taskId } = args;
        const actualTaskId = this.extractTaskId(taskId);
        if (!this.config.julesApiKey) {
        ...
        try {
            ...
            const sessionResponse = await axios.get(`https://jules.googleapis.com/v1alpha/sessions/${actualTaskId}`, { headers: { 'x-goog-api-key': this.config.julesApiKey } });
            const session = sessionResponse.data;
            // 2. Get Activities
            const activitiesResponse = await axios.get(`https://jules.googleapis.com/v1alpha/sessions/${actualTaskId}/activities?pageSize=50`, { headers: { 'x-goog-api-key': this.config.julesApiKey } });
            ...
            const events = activities
                ...
                if (a.planGenerated) {
                    ...
                    eventType =
                        Object.keys(a).find((k) => k !== 'createTime' && k !== 'name' && k !== 'description') || 'ACTIVITY';
            ...
            const patches = activities.filter((a) => a.changeSet).map((a) => a.changeSet);
            ...
            if (branch) {
                try {
                    const runListJson = await this.runGhCommand([
                        'run', 'list', '--branch', branch, '--limit', '1', '--json', 'databaseId,status,conclusion,name'
                    ]);
                    ...
                    if (runs && runs.length > 0) {
                        ...
                        if (run.conclusion === 'failure') {
                            try {
                                const logs = await this.runGhCommand(['run', 'view', run.databaseId.toString(), '--log-failed']);
                                // Truncate logs if too long
                                const truncatedLogs = logs.length > 1000 ? logs.substring(logs.length - 1000) : logs;
                                telemetryReport += `\n\n**Failed Logs (Tail)**:\n\`\`\`\n${truncatedLogs}\n\`\`\``;
                            }
                            catch (e) {
    ...
    async concludeTask(args) {
        ...
        if (!sourceFile) {
            try {
                ...
                if (found)
                    sourceFile = path.join(activeDir, found);
            }
            catch (e) { }
        }
        if (!sourceFile) {
    ...
    extractCodeReviewFromActivities(activities) {
    ...
    async bulkCreateTasks(args) {
        const { tasks } = args;
        const results = await Promise.all(tasks.map(async (taskData) => {
            try {
...
- Has Auth Cookies: ${!!this.config.googleAuthCookies}`;
    ...
    async getDependencyStatus(args) {
        const activeDir = path.resolve(process.cwd(), '.jules/active');
        const backlogDir = path.resolve(process.cwd(), '.jules/backlog');
        const archiveDir = path.resolve(process.cwd(), '.jules/archive');
        const tasks = [];
    ...
    async resolveConflicts(args) {
        ...
        try {
            ...
            try {
                ...
                return { content: [{ type: 'text', text: `No conflicts detected between ${sourceBranch} and ${targetBranch}. Safe to merge.` }] };
            }
            catch (mergeError) {
                // Step 3: Conflicts detected, extract paths
                const statusOut = await this.runGitCommand(['status', '--porcelain']);
                const unmergedPaths = statusOut.split('\n')
                    .filter(line => line.startsWith('UU ') || line.startsWith('U ') || line.startsWith(' U'))
...
`;
                ...
                await this.runGitCommand(['merge', '--abort']).catch(() => { });
                results.push(`ℹ Aborted local merge. You can now delegate the resolution task to Jules: \`/jclaw.delegate ${taskName}\``);
            }
        }
        catch (e) {
            throw new Error(`Conflict Resolution Engine Error: ${e.message}`);
        }
        return { content: [{ type: 'text', text: `## Merge Conflict Resolution\n\n${results.join('\n')}` }] };
    }
    async syncGhSessions(args) {
